...
public class MasterServer extends ThriftServer {
            ...
            public void run() {
                try {
                    while (client.getServiceState() == Service.STATE.STARTED
                        ...
                        List<ContainerStatus> completedContainers = allocResponse
                                .getCompletedContainersStatuses();

                        if (completedContainers.size() > 0
                                && client.supervisorsAreToRun()) {
                            LOG.debug("HB: Containers completed ("
    ...
    public static void main(String[] args) throws Exception {
        ...
        StormAMRMClient rmClient = new StormAMRMClient(appAttemptID,
                storm_conf, hadoopConf);
    ...
    private MasterServer(@SuppressWarnings("rawtypes") Map storm_conf,
        ...
        super(storm_conf, new Processor<StormMaster.Iface>(handler), Utils
                .getInt(storm_conf.get(Config.MASTER_THRIFT_PORT)));
        try {
            _handler = handler;
            _handler.init(this);
